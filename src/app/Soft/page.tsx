...
export default function Page() {
  ...
  // 检查是否需要密码验证
	useEffect(() => {
		const needPassword = siteContent.enablePasswordAccess && 
			...
			(siteContent.passwordAccessCategories || []).includes('Soft')
		
		if (needPassword) {
			// 检查是否已经验证过，并且密码没有更改
			const verified = localStorage.getItem('password_Soft') === 'verified'
			const storedHash = localStorage.getItem('password_Soft_hash')
  ...
  if (!isVerified) {
    return <PasswordVerify category="Soft" onVerify={() => setIsVerified(true)} />
  }
