...
export default function Page() {
  ...
  // 检查是否需要密码验证
	useEffect(() => {
		const needPassword = siteContent.enablePasswordAccess && 
			...
			(siteContent.passwordAccessCategories || []).includes('Games')
		
		if (needPassword) {
			// 检查是否已经验证过，并且密码没有更改
			const verified = localStorage.getItem('password_Games') === 'verified'
			const storedHash = localStorage.getItem('password_Games_hash')
  ...
  if (!isVerified) {
    return <PasswordVerify category="Games" onVerify={() => setIsVerified(true)} />
  }
